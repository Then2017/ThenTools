...
public class HttpUtil {
	...
	public static String getUrlAsString(String url, Map<String, String> params,
		...
		// 调试信息
		com.Main.ThenToolsRun.logger.log(Level.INFO,"url:" + url);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"params:" + params.toString());
		com.Main.ThenToolsRun.logger.log(Level.INFO,"encode:" + encode);
		// 发送请求
		String result = executeHttpRequest(httpGet, null);
		// 结束时间
		long t2 = System.currentTimeMillis();
		// 调试信息
		com.Main.ThenToolsRun.logger.log(Level.INFO,"result:" + result);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"consume time:" + ((t2 - t1)));
		// 返回结果
		return result;
	}
	...
	public static String postUrlAsString(String url,
		...
		com.Main.ThenToolsRun.logger.log(Level.INFO,"url:" + url);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"params:" + params.toString());
		com.Main.ThenToolsRun.logger.log(Level.INFO,"reqHeader:" + reqHeader);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"encode:" + encode);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"result:" + result);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"consume time:" + ((t2 - t1)));
		// 返回结果
		...
		if (params != null) {
			...
			for (String name : params.keySet()) {
				...
				try {
					...
					com.Main.ThenToolsRun.logger.log(Level.WARNING,"URLEncoder Error,encode=" + encode + ",param="
							+ params.get(name), e);
				}
				flag = "&";
			}
	...
	private static HttpPost getHttpPost(String url, Map<String, String> params,
		...
		if (params != null) {
			...
			try {
				...
				com.Main.ThenToolsRun.logger.log(Level.WARNING,"UrlEncodedFormEntity Error,encode=" + encode
						+ ",form=" + form, e);
			}
		}
		return httpPost;
	...
	private static String executeHttpRequest(HttpUriRequest request,
		...
		try {
			...
			// 如果失败
				...
				com.Main.ThenToolsRun.logger.log(Level.INFO,"HttpResonse Error:" + errorMsg);
			}
		} catch (Exception e) {
			com.Main.ThenToolsRun.logger.log(Level.WARNING,"http连接异常", e);
			throw new Exception("http连接异常");
		} finally {
			try {
				client.getConnectionManager().shutdown();
			} catch (Exception e) {
				com.Main.ThenToolsRun.logger.log(Level.WARNING,"finally HttpClient shutdown error", e);
			}
		}
		return result;
	}
	...
	public static boolean downloadFile(String path, String url) throws IOException {
		com.Main.ThenToolsRun.logger.log(Level.INFO,"path:" + path);
		com.Main.ThenToolsRun.logger.log(Level.INFO,"url:" + url);
		HttpClient client = null;
		try {
			// 创建HttpClient对象
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				try {
					...
					com.Main.ThenToolsRun.logger.log(Level.WARNING,"保存文件错误,path=" + path + ",url=" + url, e);
					return false;
				} finally {
					try {
						if (bw != null)
						...
						com.Main.ThenToolsRun.logger.log(Level.WARNING,"finally BufferedOutputStream shutdown close",e);
						return false;
					}
				}
				return true;
			...
			// 如果失败
				...
				com.Main.ThenToolsRun.logger.log(Level.WARNING,"HttpResonse Error:" + errorMsg);
			}
		} catch (ClientProtocolException e) {
			com.Main.ThenToolsRun.logger.log(Level.WARNING,"下载文件保存到本地,http连接异常,path=" + path + ",url=" + url, e);
			throw e;
		} catch (IOException e) {
			com.Main.ThenToolsRun.logger.log(Level.WARNING,"下载文件保存到本地,文件操作异常,path=" + path + ",url=" + url, e);
			throw e;
		} finally {
			try {
				client.getConnectionManager().shutdown();
