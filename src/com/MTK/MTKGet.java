...
public class MTKGet {
	...
	public boolean filepathexist(String timeornone,String happentime){
		...
		if(timeornone.equals("None")){
			logfolder=com.Main.ThenToolsRun.ThenLogfile+"/MTKPlatform/PCtime"+PCtime+"/MTKLog_nonetime_"+nonetime;
			Mainlog=logfolder+"/MTKLogger/";
			Otherlog=logfolder+"/Otherlog";
		}else{
			logfolder=com.Main.ThenToolsRun.ThenLogfile+"/MTKPlatform/PCtime"+PCtime+"/MTKLog_happentime_"+happentime;
			Mainlog=logfolder+"/MTKLogger/";
			Otherlog=logfolder+"/Otherlog";
		}
		File file=new File(Mainlog);
	...
	public String getlog(){
		...
		//getdmesg24
//		List<String> timelist=Excute.returnlist("date",2,true );
...
//		}
		...
		com.Main.ThenToolsRun.logger.log(Level.INFO,"get mtklog from phone");
		returnstr="OK";
		return returnstr;
	}
	...
	class GetlogThread implements Runnable {
		...
		public void run() {
			...
			if(errorgetlog.equals("nohappentime")){
				...
				String time=happentime.substring(0, 2)+"-"+happentime.substring(2, 4)+" "+happentime.substring(5, 7)+":"+happentime.substring(8, 10);
				JOptionPane.showMessageDialog(com.Main.ThenToolsRun.mainFrame, "No happentime : "+time+" in MTKLog, pls try correct time again! ",
						"Message", JOptionPane.ERROR_MESSAGE);
				com.Main.ThenToolsRun.logger.log(Level.INFO,"No happentime : "+time+" in MTKLog, pls try correct time again! ");
