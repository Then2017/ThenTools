...
public class MTKhappentimeUI extends JFrame {
   ...
   // MTKADBRoot mtkadbroot=new MTKADBRoot();
	...
	private JButton btnSynUe;
	/**
	 * Create the frame.
	 */
	public MTKhappentimeUI() {
		setResizable(false);
		...
		btnOK.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(com.Main.ThenToolsRun.platform.contains("msm")){
					JOptionPane.showMessageDialog(contentPane, "Only work for MTK platform!", 
							"Messge", JOptionPane.ERROR_MESSAGE); 	
					return;
				}
				//root
				if(isroot){
					JOptionPane.showMessageDialog(contentPane, "inputs *#*#13646633#*#*->Log and Debugging->User2Root->Root,run root.", 
...
//					mtkadbroot.main();
				...
				if(mtkGet.filepathexist(timeornone, happentime)){
					int confirm=JOptionPane.showConfirmDialog(contentPane, mtkGet.getMainlog()+"     is exist!\n"+
		...
		chckbxRoot.setBounds(312, 155, 103, 23);
		...
		//Syn UE time
		btnSynUe = new JButton(getString("btnSynUe"));
		btnSynUe.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String[] time=mtkGet.getUEtime();
				if(time[1]!=null){
				Mon=time[1];
				Day=time[2];
				Hour=time[3];
