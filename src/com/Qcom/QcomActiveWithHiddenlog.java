...
public class QcomActiveWithHiddenlog{
	   ...
	   // private SimpleDateFormat phoneDateFormat = new SimpleDateFormat("mm_ss_SSS");
		...
		public void start() {
			...
			com.Main.ThenToolsRun.logger.log(Level.INFO,"active log with userdebug");		
		}
		...
		//open hidden log
		public void OpenHiddenLog() {
			//remount
			Excute.execcmd("remount", 3, true);
			//setporp
			//Telecom InCall ContactsPref Telephony Mms
			String loginfo=qcomHiddenLogSettings.getLoginfo();
			loginfo=loginfo.replace("\n", "");
			String[] loginfosplit=loginfo.split(";");
			for(String str:loginfosplit){
				String temp=Excute.execcmd2(str);
					com.Main.ThenToolsRun.logger.log(Level.INFO,"open :"+str+"  result: "+temp);
			}
			//
		}
		//adb restart
		public void StopandStart(){
			Excute.execcmd2("stop");
			Excute.execcmd2("start");
			try {
				Thread.sleep(qcomHiddenLogSettings.getWaittime());
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				com.Main.ThenToolsRun.logger.log(Level.WARNING,e.toString());LoggerUtil.printException(e);
			}
		...
		class ActivelogThread implements Runnable {
			...
			public void run() {
				...
				com.Main.ThenToolsRun.logger.log(Level.INFO,"start to open hidden log");
				com.Main.ThenToolsRun.mainFrame.progressBarmain.setValue(20);//******************
				if (!Adbroot()){
					com.Main.ThenToolsRun.mainFrame.progressBarmain.setValue(0);//******************
					JOptionPane.showMessageDialog(com.Main.ThenToolsRun.mainFrame, "Auto root failed, pls try again or root by handle!", 
							"Message", JOptionPane.ERROR_MESSAGE); 
					com.Main.ThenToolsRun.logger.log(Level.INFO,"Auto root failed, pls try again or root by handle when open hidden log!");
					return;
				}
				OpenHiddenLog();
				...
				if (CheckUE.getisroot().equals("Yes")){
					...
					com.Main.ThenToolsRun.mainFrame.progressBarmain.setValue(0);//******************
					JOptionPane.showMessageDialog(com.Main.ThenToolsRun.mainFrame, "Device must root first!", 
							"Message", JOptionPane.ERROR_MESSAGE); 
					com.Main.ThenToolsRun.logger.log(Level.INFO,"Device must root first, and then can open hidden log!");
					return;
//					activeadb();
//					com.Main.ThenToolsRun.mainFrame.progressBarmain.setValue(50);//******************
//					if(checkdmesg()){
...
//					Excute.execcmd("shell <"+com.Main.ThenToolsRun.datalocation+"/tempdata",3,false);
				...
				if(checklogfile()){
					...
					com.Main.ThenToolsRun.logger.log(Level.INFO,"Active "+whatlog+" logs for successfully with hidden log!");
					JOptionPane.showMessageDialog(com.Main.ThenToolsRun.mainFrame, "Active "+whatlog+" logs for successfully with hidden log!", 
